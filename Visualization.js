    ...
    constructor(config) {
        ...
        this.colors = {
            ...
            text: {
                ...
                weight: config?.colors?.text?.weight || "#4040a1"
            }
        }
    ...
    present = (data = {}) => {
        ...
        data.l.forEach((layer, lindex) => {
            ...
            neurons.forEach((neuron, nindex) => {
                ...
                // Draw neuron.
                this.draw("circle", { cx: x, cy: y, r, fill: this.colors.neuron.fill, stroke: this.colors.neuron.stroke, "stroke-width": 2 })
                // Draw neuron bias.
                this.draw("text", { x, y, fill: this.colors.text.bias, "text-anchor": "middle", "dominant-baseline": "middle", "font-size": text }).textContent = neuron.b.toFixed(5)
                map[neuron["#"]] = { x, y, r }
            })
        })
        data.c.forEach(connection => {
            const from = !isNaN(connection["<"]["#"]) ? connection["<"]["#"] : connection["<"]
            const to = !isNaN(connection[">"]["#"]) ? connection[">"]["#"] : connection[">"]
            if (!connection.s || !data.n[from].s || !data.n[to].s) return
            const id = `${from}${to}`
            const curve = r * 3
            const x1 = map[from].x + map[from].r
            const y1 = map[from].y
            const x2 = map[to].x - map[to].r
            const y2 = map[to].y
            const cx1 = x1 + curve
